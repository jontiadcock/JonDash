...
silently discarded on the way into the build. The add-on looked correct in its own source, passed
every check, and rendered wrong, with no error anywhere to notice.

**It was also teaching people to do the wrong thing.** JonDash's own guidance for making widgets
adapt to their size points authors at exactly the technique that was broken. Reported by the add-ons
repository, who lost time to it.

**Documented for add-on authors:** the tile a widget sits in can be measured for its **width only**,
never its height — which matters on a wide, short tile, where "plenty of room" gets reported at the
moment there is none. The authoring guide now says so, and gives the layout pattern that works
instead.
