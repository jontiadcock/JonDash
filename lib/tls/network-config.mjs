...
/*
 * ⚠ Honours JONDASH_DATA_DIR, and it must. This file decides the HTTP port, so a build resolving
 * the wrong .data binds the wrong one — and the default is 3000, which on a developer machine is
 * somebody's live instance. It used to hardcode `process.cwd()` and was the only data-directory
 * resolver in the app that ignored the variable.
 *
 * ⚠ Read ONCE at module load, unlike `lib/config.ts` which resolves per call. That covers the
 * case that matters — a spawned server has the variable set before it starts — but changing it
 * mid-process does NOT move these paths; they would have to become functions, across ~34 call
 * sites in .mjs files with no typechecking.
 * REFS lib/config.ts › dataDir() — keep the fallback identical to it
 */
const DATA_DIR = process.env.JONDASH_DATA_DIR || path.join(process.cwd(), ".data");
export const NETWORK_FILE = path.join(DATA_DIR, "network.json");
/** REFS lib/tls/acme.mjs · lib/tls/certs.mjs · lib/tls/challenge-store.mjs */
export const TLS_DIR = path.join(DATA_DIR, "tls");
export const STATUS_FILE = path.join(TLS_DIR, "status.json");
/** REFS lib/tls/acme.mjs */
export const ACCOUNT_KEY_FILE = path.join(TLS_DIR, "account.key");
/** REFS lib/tls/acme.mjs · lib/tls/certs.mjs */
export const CERT_FILE = path.join(TLS_DIR, "cert.pem"); // fullchain
/** REFS lib/tls/acme.mjs · lib/tls/certs.mjs */
export const KEY_FILE = path.join(TLS_DIR, "privkey.pem");

/** REFS app/admin/modules/rebuild-watch.tsx */
export const MODES = /** @type {const} */ (["off", "letsencrypt", "selfsigned", "byo"]);

/**
 * @typedef {{ mode: "off"|"letsencrypt"|"selfsigned"|"byo", httpPort: number, httpsPort: number,
 *   domain: string, email: string, certPath: string, keyPath: string, selfSignedDays: number
 * }} NetworkConfig
 */
...
export const DEFAULTS = {
  ...
  domain: "",
  email: "",
  // Kept after the 1.8.0 import rework: an install already serving a BYO cert from these paths
  // must keep serving it. New imports write into .data/tls/ and leave these empty.
  certPath: "",
  keyPath: "",
  selfSignedDays: 365,
};

function coerceDays(value, fallback) {
...
/**
 * REFS app/admin/network/actions.ts · app/admin/network/page.tsx · lib/tls/network.ts
...
function normalize(raw) {
  ...
  return {
    ...
    selfSignedDays: coerceDays(raw?.selfSignedDays, DEFAULTS.selfSignedDays),
  };
}
...
export function writeNetworkConfig(cfg) {
  ...
  const clean = {
    ...
    selfSignedDays: coerceDays(cfg.selfSignedDays, DEFAULTS.selfSignedDays),
  };
...
/** REFS app/admin/network/page.tsx · lib/tls/network.ts */
...
/** REFS app/admin/network/actions.ts · lib/tls/acme.mjs */
