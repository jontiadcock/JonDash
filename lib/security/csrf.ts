 ...
 * cookie used to be `SameSite=Strict` and was a second barrier; it is `lax` now, because Strict also
 * withheld the cookie on ordinary top-level navigation and sent people to a login page they did not
 * need (BUG-73). `lax` still withholds it on cross-site POSTs, which is the vector.
 *
 * **So this function has to fail closed, and does:** the request's Origin — or, when a browser omits
 * it, the Referer — must match the host the request was sent to. **Neither present means refused**,
 * rather than allowed. Do not "simplify" that into a permissive fallback: with the cookie relaxed,
 * this is what stands between a mutating action and a cross-site caller.
 *
 * ## Related code
