...
- Tests run on **Vitest** against a throwaway SQLite database that is created,
  migrated, and deleted automatically for each run.
...
authorization, backup export/restore (including encryption), backup codes, and
settings — so changes that break them fail fast.
