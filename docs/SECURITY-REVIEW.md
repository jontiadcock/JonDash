...
| 7 | Info | CSP uses `style-src 'unsafe-inline'`; the icon route's stricter CSP is overridden by the global CSP (nosniff still applies). | Tighten later (hashed styles; let the icon route's headers win). Low risk. | ⛔ **Still open** — tracked as **BUG-50** — COOP/CORP **fixed 1.8.5-beta.1**; `style-src 'unsafe-inline'` still open |
